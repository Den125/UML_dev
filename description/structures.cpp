...
void savePrecedent(QString name, precedentData data)
{
     ...
     key_elem.insert("precondition",data.predUsl);
     for (int i=0;i<data.seqVect.count();i++)
     {
         QJsonObject seq;
         seq.insert("type_seq",type_seq(data.seqVect[i].type));
         seq.insert("condition_seq",data.seqVect[i].usl);
...
precedentData loadPrecedent(QString name)
{
    ...
    if (!keys.empty())
    {
        new_precedent.predUsl=jsvalue.toObject().value("precondition").toString();
        ...
        for (;index_seq<keys.count()-3;index_seq++)
        {
            ...
            seq.usl=seq_list.toObject().value("condition_seq").toString();
            int index_elem=0;
